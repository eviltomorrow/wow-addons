...
| 功能 | 实现方式 | 默认 |
| --- | --- | --- |
| 模型/特效还原 | 插件在游戏内设置 `overrideArchive=0`（持久化到 WTF），重启游戏生效 | 关闭（手动开启） |
| 图标还原 | 需自行把未和谐图标放入 `_retail_/Interface/ICONS/`，插件不涉及 | — |

本插件做法：
- 默认**关闭**，避免拉取器每次启动时因 `overrideArchive=0` 触发"重新应用设置"的读条更新；
- 需要时手动开启：设置面板勾选"模型/特效反和谐"或 `/ah harmony on`，插件写入 `overrideArchive=0` 并持久化到 WTF，重启游戏生效；
- 若想彻底摆脱拉取器重置，可把 `_retail_/WTF/Config.wtf` 设为**只读**（代价：只读期间游戏内其它设置无法保存）；
...
| 物品装等显示 | 背包物品、角色面板穿戴装备图标上叠加显示装等 | 开启 |
...
- **背包物品**：在背包/银行格子图标右下角叠加显示物品装等（白色描边数字）；
- **角色面板**：在 C 键角色面板每个穿戴装备图标上同样叠加显示装等；
- 用 `hooksecurefunc("SetItemButtonQuality")` 覆盖所有物品按钮 + `PaperDollFrame` 角色面板钩子，物品数据未缓存时自动重试；不读取秘密数据；
- 默认跟随 `itemLevel` 开关，`/ah ilvl` 可切换。
...
- 原生 Raid 画质只针对团队副本，**大秘境不会自动触发**；本功能提供**手动画质预设**：`/ah mplus 0-2`（极限/标准/保守）；
- 开启时把 `RenderScale`、粒子密度、法术密度、SSAO、阴影、计算特效、投影、视距等 CVar 写入低值，并把**原值保存到 SavedVariables**，关闭（`/ah mplus off`）时一键恢复；
- 针对大米一波拉的特效压力设计，进本前手动切换即可；档位值见 `Graphics.lua` 的 `MPLUS_PRESETS` 表。
...
| `/ah ilvl [on\|off]` | 开关背包/装备装等显示（不带参数则翻转） |
| `/ah raid [on\|off\|0-9]` | 团本画质优化：数字=设定档位并开启，on/off=开关 |
| `/ah mplus [on\|off\|0-2]` | 大秘境压帧：0=极限/1=标准/2=保守，自动保存原画质可恢复 |
| `/rl` | 重载界面 |
| `/fs` | 窗口化 / 全屏切换 |
| `/qg` | 交接当前任务（接受/完成） |
...
- 按"反和谐 / 基础功能 / 显示 / 工具"四个分区展示：金色小节标题 + 分隔线分组，所有开关勾选列对齐，一目了然；
- 反和谐分区内可直接查看当前状态并一键重新检测；
- 工具分区内可一键应用 CVar 优化、恢复默认设置，底部附功能说明与命令速查。

### 模型/特效反和谐

1. 设置面板勾选"模型/特效反和谐"（或 `/ah harmony on`）——**默认关闭**，避免拉取器每次启动触发"重新应用设置"的读条更新；
2. 插件写入 `overrideArchive=0`（持久化到 `WTF/Config.wtf`）；
3. **重启游戏**后生效，`/ah check` 应显示"已开启"；
4. 若仍被拉取器每次重置：可把 `_retail_/WTF/Config.wtf` 设为**只读**（注意：只读期间游戏内其它设置无法保存）。
...
- 装等显示：`hooksecurefunc("SetItemButtonQuality") / GetItemInfo / GetInventoryItemLink / C_Container.GetContainerItemInfo / C_Timer.NewTicker`
